...
function VoiceCloneRow({ lang, config, cloneState, onClone, onTest, testState }) {
  ...
  return (
    <div style={{
      ...
      {cloneState === 'error' && (
        <div style={{ fontSize: '0.6rem', color: 'var(--mic-muted)', marginTop: '0.2rem' }}>
          Clone failed
        </div>
      )}
      {cloneState === 'complete' && (
        <div style={{ fontSize: '0.6rem', color: 'var(--mic-listening)', marginTop: '0.2rem' }}>
          Clone complete
        </div>
      )}
      {hasProfile && (
...
export default function TTSSettings({ sendCommand, onBack }) {
  ...
  const handleTestVoice = useCallback((lang) => {
    ...
    sendCommand?.({ cmd: 'tts_test', text: 'Hello, this is a voice test.', lang });
    setTimeout(() => setTestStates(prev => ({ ...prev, [lang]: 'idle' })), 5000);
  }, [sendCommand]);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontSize: '0.95rem', fontWeight: 600 }}>TTS Settings</span>
        </div>
      </div>
