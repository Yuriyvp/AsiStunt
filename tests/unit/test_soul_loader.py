...
class TestLoadSoul:
    def test_load_default_soul(self):
        ...
        assert soul.voice_method == "description"
        assert "warm alto" in soul.voice_description
        assert soul.default_language == "hr"
        assert "hr" in soul.languages
        assert "en" in soul.languages
    ...
    def test_persona_card(self):
        ...
        # backstory comes before personality
    ...
    def test_persona_card_empty_backstory(self):
        ...
        card = soul.persona_card()
        assert "You are a test bot." in card
        assert "\n\n" not in card  # no double newline from empty backstory
    ...
    def test_invalid_bad_voice_method(self):
        """Schema validation catches invalid voice method enum."""
        with pytest.raises(ValueError, match="validation failed"):
            load_soul(FIXTURES / "invalid_bad_voice_method.soul.yaml")
    ...
    def test_defaults_for_optional_fields(self):
        ...
        soul = load_soul(PROJECT_ROOT / "soul" / "default.soul.yaml")
        # These are present in default.soul.yaml but test the accessor paths
        assert isinstance(soul.summary_style, str)
        assert 0 <= soul.summary_trigger <= 1
        assert 0 <= soul.detection_threshold <= 1

    def test_voice_clone_method(self):
        """Voice clone method with reference_audio=null."""
        soul = load_soul(PROJECT_ROOT / "soul" / "default.soul.yaml")
        assert soul.voice_reference_audio is None
